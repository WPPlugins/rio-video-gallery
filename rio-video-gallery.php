...
?>
    <iframe frameborder="0" width="200" height="120" src="http://www.dailymotion.com/embed/video/<?php echo $video_id_res; ?>" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
    <?php
	}
?></td>
  </tr>
  ...
  <div class="postbox">
      <div class="inside">
        <table>
          ...
          <tr>
            <td ><input type="radio" <?php if(!empty($video_link_target) && $video_link_target == 'popup') { echo 'checked="checked"';} elseif(empty($video_link_target)) {echo 'checked="checked"';}?> value="popup" name="video_link_target" >
              Popup View</td>
            <td><input type="radio" <?php if(!empty($video_link_target) && $video_link_target == 'tab') { echo 'checked="checked"';}?> value="tab" name="video_link_target">
              Single Page</td>
	...
	if ($the_query->have_posts()):
		while ($the_query->have_posts()):
		 ...
		 if (!empty($provider_shortres) && $provider_shortres == 'youtube') {
   				...
   				$short_code_output.="<img src='http://img.youtube.com/vi/".$video_id_shortres."/0.jpg' alt='".get_the_title()."' title='".get_the_title()."' width='".$video_thumb_width_gshort."' height='".$video_thumb_height_gshort."'>";
   				 }
   		...
   		$short_code_output.="<img src='".$thumb."' alt='".get_the_title()."' title='".get_the_title()."' width='".$video_thumb_width_gshort."' height='". $video_thumb_height_gshort."'>";
   			 } else if (!empty($provider_shortres) && $provider_shortres == 'dailymotion') {
   	$short_code_output.="<img src='http://www.dailymotion.com/thumbnail/video/".$video_id_shortres."' alt='".get_the_title()."' title='".get_the_title()."' width='".$video_thumb_width_gshort."' height='".$video_thumb_height_gshort."'>";
         	 }
			...
			$short_code_output.="<header>";
				...
				$short_code_output.="<div class='poup_window' id='show_content".$postid."'>
							...
							$short_code_output.="<iframe src='//player.vimeo.com/video/".$video_id_shortres."' width='".$video_sthumb_width_res."' height='". $video_sthumb_height_res."' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>";
							 ...
							 $short_code_output.="<iframe frameborder='0' width='". $video_sthumb_width_res."' height='".$video_sthumb_height_res."' src='http://www.dailymotion.com/embed/video/".$video_id_shortres."' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>";
							 }
				 ...
				 if (!empty($vposted_date_display_gshort)) {
							 $short_code_output.="<span>".rio_video_getPostViews($post_ID)." Views</span> <span>".human_time_diff( get_the_time('U'), current_time('timestamp') ) . ' ago';"</span>";
				 }
			...
			$category=strip_tags($category);
				 ...
				 $short_code_output.="<span class='category_name'>".$category."</span>&nbsp;";
					  ...
					  if(!empty($vposted_date_display_gshort)){
                        $short_code_output.="<span>".rio_video_getPostViews($post_ID)." Views</span>";
...
<?php if (!empty($video_id_pres)) { ?>
  <figure>
   ...
   <iframe frameborder="0" width="<?php echo $video_sthumb_width_pres; ?>" height="<?php echo $video_sthumb_height_pres; ?>" src="http://www.dailymotion.com/embed/video/<?php echo $video_id_pres; ?>" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
   <?php } ?>
  </figure>
  <?php } ?>
<article>
	...
	if ($post->post_type == 'video-gallery') {
        ...
        <figure>";
			if (!empty($provider_shortres) && $provider_shortres == 'youtube') {
				...
				$single_template.= "<iframe frameborder='0' width='" . $video_sthumb_width_res . "' height='" . $video_sthumb_height_res . "' src='http://www.dailymotion.com/embed/video/" . $video_id_shortres . "' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>";
			}
